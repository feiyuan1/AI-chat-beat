...
import { AdaptResult, StorePayload } from "../types";
import {
...
const extractUserPrompt = (messages: unknown): string | null => {
  ...
  const text = parts.find((p: unknown) => typeof p === "string" && p.trim()) as
    | string
    | undefined;
  if (!text) {
    return null;
...
export const ChatDataAdapter = createAdapterErrorBoundary<
  AdaptResult<StorePayload>
>("chatgpt-chatdata", (data: any) => {
