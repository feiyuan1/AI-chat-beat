...
import { LocalStorageData, LocalStoragekeys } from "./types";
...
export const setLocalStorage = <Key extends LocalStoragekeys>(
  ...
  const data = Object.assign({}, oldData && JSON.parse(oldData), {
    [key]: value,
  });
  localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
};

export const removeLocalStorageKey = <Key extends LocalStoragekeys>(
  key: Key,
) => {
  const oldData = localStorage.getItem(STORAGE_KEY);
  if (!oldData) {
